...
%to get figures at the top of pages
\makeatletter
\setlength{\@fptop}{0pt}
\makeatother

% to get abstract and title on the same page
\usepackage{titling}
...
\begin{abstract}
Shor discovered in 1995 \cite{Shor} that quantum computers powerful enough could break all cryptographic primitives based on discrete logarithm and integer factorization such as the widespread RSA and commonly used elliptic curve cryptography.  Ever since, efforts have been made to find quantum-safe cryptographic primitives. Since the foundational works of Couveignes in the end of the 1990's \cite{Couveignes} and the discovery of Supersingular Isogeny Diffie-Hellman (SIDH) by De Feo, Jao and Pl\^{u}t in 2011~\cite{DeFeoSIDH}, isogeny-based cryptography has become a promising area of research in post-quantum cryptography.   Oriented Supersingular Isogeny Diffie-Hellman (OSIDH) is a key exchange protocol due to Kohel and Col\`{o} \cite{OSIDH}, generalizing the ideas of Commutative Supersingular Isogeny Diffie-Hellman (CSIDH) due to Castryck et. al.  (2018) \cite{CSIDH}, itself inspired from Couveignes' cryptosystem.  The goal of this master's thesis is to study OSIDH. We conduct a cryptanalysis and propose an original attack running on a classical computer. We implement this attack in \verb?SageMath? \cite{sagemath} with toy parameters. We come to the conclusion that OSIDH is not secure if its parameters are not chosen carefully. Unfortunately, a secure choice of parameters impedes new cryptographic constructions based on the OSIDH framework.
\end{abstract}
...
Let $(E, \iota_E)$ and $(F,\iota_F)$ be two $K$-oriented elliptic curves.  An isogeny $\varphi : E\longrightarrow F$ is $K$-\emph{oriented} if $\varphi_*(\iota_E)=\iota_F$.  We denote this by $\varphi : (E, \iota_E)\longrightarrow(F,\iota_F)$.
\end{Definition}
...
Taking a generator $\omega$ of $\mO$ ($\mO=\Z[\omega]$),  we get that $m\deg(\varphi)\omega=a+bm'\omega$ for certain integers $a,b\in\Z$.  It follows that $a=0$ and $m\deg(\varphi)=bm'$ so that $m'|\deg(\varphi)$ because $m$ and $m'$ are coprime.  By symmetry,  $m|\deg(\varphi)$,  so $mm'|\deg(\varphi)$.
\end{proof}

\begin{Proposition}\label{Proposition 4}
We assume that $\ell:=\deg(\varphi)$ is a prime number.  Then one of the following statements holds:
\begin{description}
...
%Since $\ell$-isogenies whose domain is a vertex $(E,\iota)$ of $G_\ell(K,p)$  are separable, they are determined by their kernel (by \cite[Corollary III.4.11]{Silverman1}) which is a cyclic subgroup of order $\ell$ in $E[\ell]$. Since there are $\ell+1$ such subgroups, the graph is $(\ell+1)$-regular, if we omit the fact that $\ell$-isogenies with different kernels may be identified up to $K$-isomorphism.
...
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm,scale=1.6]
\clip(-3.9,-4.5) rectangle (3.9,1);
\draw [line width=1pt] (3.5,-4) circle (0.3cm);
\draw [line width=1pt] (0,0) circle (0.3cm);
\draw [line width=1pt] (-1.5,-4) circle (0.3cm);
\draw [line width=1pt] (-3.5,-4) circle (0.3cm);
\draw [line width=1pt] (0.5,-4) circle (0.3cm);
\draw [line width=1pt] (2,-2) circle (0.3cm);
\draw [line width=1pt] (1.5,-4) circle (0.3cm);
\draw [line width=1pt] (-0.5,-4) circle (0.3cm);
\draw [line width=1pt] (-2.5,-4) circle (0.3cm);
\draw [line width=1pt] (-1,-3) circle (0.3cm);
\draw [line width=1pt] (-3,-3) circle (0.3cm);
\draw [line width=1pt] (0,-1) circle (0.3cm);
\draw [line width=1pt] (-2,-2) circle (0.3cm);
\draw [line width=1pt] (3,-3) circle (0.3cm);
\draw [line width=1pt] (1,-3) circle (0.3cm);
\draw [line width=1pt] (2.5,-4) circle (0.3cm);
...
\caption{Representation of a connected component (with volcano structure) of $G_2(\Q(i),79)$, the $\Q(i)$-oriented supersingular $2$-isogeny graph over $\F_{79^2}$ up to depth $4$. Here, $a$ is the generator of $\F_{79^2}/\F_{79}$ and satisfies $a^2-a+3=0$.}\label{Figure 4}
\end{figure}
...
\draw [line width=1pt] (0,0) circle (0.3cm);
\draw [line width=1pt] (0,-1) circle (0.3cm);
\draw [line width=1pt] (1.5,-1) circle (0.3cm);
\draw [line width=1pt] (-1.5,-1) circle (0.3cm);
\draw [line width=1pt] (1.5,-3) circle (0.3cm);
\draw [line width=1pt] (-1.5,-3) circle (0.3cm);
\draw [line width=1pt] (0,-2) circle (0.3cm);
\draw [line width=1pt] (0,-0.3)-- (0,-0.7);
...
\caption{Action of the prime ideal $\mfq_j$ on the descending $\ell$-isogeny chain.}\label{Figure 1}
\end{figure}
...
\caption{Naive Oriented Supersingular Isogeny Diffie-Hellman key exchange protocol. Public data in black, Alice's secret data in green, Bob's secret data in blue, shared secret in red.}\label{Figure 2}
\end{figure}
...
The real OSIDH protocol as introduced in \cite[§ 5.2]{OSIDH} is presented in Figure \ref{Figure 3}. 
...
\begin{tikzpicture}
  % Public parameter:
  \node[draw=none,fill=none,align=center] (public) at (0,1) {Public parameters:\\ $(E_i,\iota_i)_{0\leq i\leq n}, \ \mf{q}_1, \cdots, \mf{q}_t$};
  ...
  \node[draw=none,fill=none,anchor=east] (asecret) at ($(Alice) + (0,-1)$) {$e_1,\cdots, e_t\in_R\i{-r}{r}$};
  \node[draw=none,fill=none,anchor=east] (asecretideal) at ($(Alice) + (0,-1.75)$) {$\mf{a}:=\prod_{j=1}^t\mf{q}_j^{e_j}$};
  \node[draw=none,fill=none,anchor=east] (Apublic) at ($(Alice) + (0,-3)$) {$(E_{A,i},\iota_{A,i}):=[\mf{a}]\cdot (E_i,\iota_i)$};
  \node[draw=none,fill=none,anchor=east] (akey) at ($(Alice) + (0,-5)$) {$E_{AB,n}:= [\mf{a}]\cdot E_{B,n}$};
  ...
  % Calculations of Bob
  \node[draw=none,fill=none,anchor=west] (bsecret) at ($(Bob) + (0,-1)$) {$f_1,\cdots, f_t\in_R\i{-r}{r}$};
   \node[draw=none,fill=none,anchor=west] (bsecretideal) at ($(Bob) + (0,-1.75)$) {$\mf{b}:=\prod_{j=1}^t\mf{q}_j^{f_j}$};
  \node[draw=none,fill=none,anchor=west] (Bpublic) at ($(Bob) + (0,-4)$) {$(E_{B,i},\iota_{B,i}):=[\mf{b}]\cdot (E_i,\iota_i)$};
  \node[draw=none,fill=none,anchor=west] (bkey) at ($(Bob) + (0,-5)$) {$E_{AB,n}:= [\mf{b}]\cdot E_{A,n}$};
   
  % Messages
  \draw[->,thick] ($(Alice)+(0,-3)$) -- ($(Bob)+(0,-3)$) node [pos=0.5,above,font=\footnotesize] {$[\mfq_j]^{-r}\cdot E_{A,n}\longrightarrow \cdots\longrightarrow [\mfq_j]^{r}\cdot E_{A,n} \ (1\leq j\leq t)$};
  \draw[->,thick] ($(Bob)+(0,-4)$) -- ($(Alice)+(0,-4)$) node [pos=0.5,above,font=\footnotesize] {$[\mfq_j]^{-r}\cdot E_{B,n}\longrightarrow \cdots\longrightarrow [\mfq_j]^{r}\cdot E_{B,n} \ (1\leq j\leq t)$};
    
\end{tikzpicture}

\caption{The OSIDH protocol as presented in \cite[§ 5.2]{OSIDH}.}\label{Figure 3}
\end{figure}
...
Actually, this problem can be solved in polynomial time using the following algorithm. This algorithm is based on the Deuring correspondence between integral left $\m{R}_0$-ideals and isogenies with domain $E_0$ (see Appendix \ref{Paragraph 2}), on an algorithm explicitly computing this correspondence (see Appendix \ref{Paragraph 4}) and on a quaternion arithmetic algorithm due to Kohel, Lauter, Petit and Tignol \cite{KLPT} (see Appendix \ref{Paragraph 3}). 
...
Find an equivalent ideal $J\sim I$ of powersmooth norm using KLPT (see Appendix \ref{Paragraph 3}, Algorithm \ref{Algorithm 2})\;
Compute the isogeny $\phi:E_0\longrightarrow E$ of kernel $E_0[J]$ using effective Deuring correspondence (see Appendix \ref{Paragraph 4}, Algorithm \ref{Algorithm 4})\;
Return $E$\;
...
Knowing $\ell$-concise representations $\m{R}$ and $\m{R}'$ (as maximal order of $B_{p,\infty}$) of $\End(E_n)$ and $\End(F_n)$ respectively, we obtain an $\ell$-concise representation of their product $I:=\m{R}\m{R}'$ which is a connecting left-$\m{R}$-ideal, meaning that:
\[O_L(I):=\{\alpha\in B_{p,\infty}\mid \alpha\cdot I \subseteq I\}=\m{R} \quad \mbox{and} \quad O_R(I)=\{\alpha\in B_{p,\infty}\mid I\cdot\alpha \subseteq I\}=\m{R}'.\]
To make $I$ integral (\ie such that $I\subseteq \m{R}$) we may multiply this basis by an integer of size $O(\log(p))$, so that the basis remains $\ell$-concise. We compute a $\Z$-basis of $I$ in polynomial time in $\log(p)$ by computing the hermite normal form of the matrix of a generating set of $I$ in a basis of $\m{R}$. 
...
Let $N, n\in\N^*$ and $\m{I}_{N,n}$ be the set of full-rank sublattices of $\Z^n$ of covolume $N$.

\begin{Lemma}\label{Lemma 22}
\begin{description}
\item[(i)] $\m{I}_{N,n}$ is finite.
\item[(ii)] Let $\Lambda$ be a random variable following the uniform distribution on $\m{I}_{N,n}$. Then, for all $\varepsilon>0$, there exists $n_0, N_0\in\N^*$ such that for all $n\geq n_0$ and $N\geq N_0$:
...
\textbf{(ii)} Let $\varphi(1):=(\overline{a},\overline{b})$, with $a,b\in\i{0}{d-1}$. Since $\varphi$ is injective, $\varphi(1)$ has order $d$ so $a, b$ and $d$ are coprime. As a consequence, there exists $u,v\in\i{0}{d-1}$ such that $au+bv\equiv 1 \ [d]$. As a consequence,
\[(\overline{x},\overline{y})\in(\Z/d\Z)^2\longmapsto (\overline{a}\overline{x}-\overline{v}\overline{y},\overline{b}\overline{x}+\overline{u}\overline{y})\in(\Z/d\Z)^2\]
is an automorphism of $\Z$-modules because its matrix in the canonical basis of $(\Z/d\Z)^2$ has determinant $\overline{au+bv}=1$. It follows that
\[(\Z/d\Z)^2=\Z(\overline{a},\overline{b})\oplus\Z(-\overline{v},\overline{u})=\im(\varphi)\oplus\Z(-\overline{v},\overline{u}),\]
so that $(\Z/d\Z)^2/\im(\varphi)\simeq \Z(-\overline{v},\overline{u})\simeq \Z/d\Z$.

\textbf{(iii)} Let $a,b\in\Z$ such that $\varphi(1)=(\overline{a},\overline{b})$.  Since $\varphi$ is injective, $\varphi(1)$ has order $\ell^{n-1}$ so $\ell^{n-1}\overline{b}=0$ \ie $\ell^n|\ell^{n-1}b$ \ie $\ell|b$. So we may write $a:=\ell^e a'$ and $b:=\ell^f b'$ with $a'$ and $b'$ prime to $\ell$, and $(e,f)\in\N\times\N^*$.  It follows that
\[\ell^{n-1}=|\varphi(1)|=\lcm(|\overline{a}|,|\overline{b}|)=\lcm(\ell^{n-1-e},\ell^{n-f})=\ell^{\max(n-1-e,n-f)},\]
so that $\max(n-1-e,n-f)=n-1$. If $e=0$, then $\overline{a}$ generates $\Z/\ell^{n-1}\Z$, so
\[(\Z/\ell^{n-1}\Z)\times(\Z/\ell^{n}\Z)=\im(\varphi)\oplus \{0\}\times\Z/\ell^{n}\Z,\]
and we immediately conclude that the quotient is isomorphic to $\Z/\ell^{n}\Z$.
...
Find a solution $v\in\i{0}{2u}$ to $v^2\equiv d \ [4u]$, \eg using Tonelli-Shanks algorithm \cite[Algorithm 1.5.1]{Cohen1} when $u$ is prime to find a square root mod $u$, and conclude by Chinese remainder theorem. If $d$ is not a square mod $4u$, return False\; 
...
\section{The KLPT algorithm}\label{Paragraph 3}

Let $p$ be a prime number. In this section, we shall mean polynomial in $\log(p)$ every time we use the term polynomial. 

We fix $\m{R}$ a concise maximal order of $B_{p,\infty}$ and $I\subseteq \m{R}$ an integral and concise left $\m{R}$-ideal (as defined in Definition \ref{Definition 3}).  The KLPT algorithm due to Kohel, Lauter, Petit and Tignol \cite{KLPT} finds an equivalent integral ideal $J\sim I$ of powersmooth norm in polynomial time (in $\log(p)$). The main idea of this algorithm lies in the following lemma:
...
As previously announced, the following algorithm works under the hypothesis that $\m{R}$ is special but it can be generalized by using a connecting ideal to a special order and two instances of this algorithm (see \cite[Theorem 9]{KLPT}).
...
$L$ has full rank since $G\simeq \Z^t/L$ is finite so we may apply point (iii) of the previous lemma to get:
\[L^*=\sum_{k=1}^r L_k^*.\]
...
\subsection{Principle of the sieve: producing new states}\label{Paragraph 12}
