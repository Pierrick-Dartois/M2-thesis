...
\begin{abstract}
Shor discovered in 1995 \cite{Shor} that quantum computers powerful enough could break all cryptographic primitives based on discrete logarithm and integer factorization such as the widespread RSA and commonly used elliptic curve cryptography.  Ever since, efforts have been made to find quantum-safe cryptographic primitives. Since the foundational works of Couveignes in the end of the 1990's \cite{Couveignes} and the discovery of Supersingular Isogeny Diffie-Hellman (SIDH) by De Feo, Jao and Pl\^{u}t in 2011 \cite{DeFeoSIDH}, isogeny-based cryptography has become a promising area of research in post-quantum cryptography.   The Oriented Supersingular Isogeny Diffie-Hellman (OSIDH) due to Kohel and Col\`{o} \cite{OSIDH} is a key exchange protocol generalizing the ideas of Commutative Supersingular Isogeny Diffie-Hellman (CSIDH) due to Castryck et. al.  (2018) \cite{CSIDH}, itself inspired from Couveignes' cryptosystem.  The goal of this master's thesis is to study OSIDH. We conduct a cryptanalysis and propose an original attack running on a classical computer. We implement this attack with \verb?SageMath? \cite{sagemath} for toy parameters. We come to the conclusion that OSIDH is not secure if its parameters are not chosen carefully. Unfortunately, a secure choice of parameters impedes new cryptographic constructions based on the OSIDH framework.
\end{abstract}
...
I also would like to thank the IBM cryptography team for their warm welcome. I think about Romain Gay, Jonathan Bootle, Julia Hesse, Bertram Poettering, Vadim Lyubashevsky and Michael Osborne in particular. I am also very grateful to my professor Elisa Lorenzo Garcia for her strong support.
...
\item[(ii)] If $\ell|[\mO_K:\mO]$, then there are no horizontal, $\ell/[\mO^\times:(\Z+\ell\mO)^\times]$ descending and one ascending $\ell$-isogenies.
\end{description}
...
\begin{tikzpicture}
  % Public parameter:
  \node[draw=none,fill=none,align=center] (public) at (0,1) {Public parameter:\\ $(E_i,\iota_i)_{0\leq i\leq n}, \ \mf{q}_1, \cdots, \mf{q}_t$};
  ...
  % Calculations of Bob
  \node[draw=none,fill=none,anchor=west] (bsecret) at ($(Bob) + (0,-1)$) {$e_1,\cdots, e_t\in_R\i{-r}{r}$};
...
Let $p$ be a prime number. In this section, we will mean polynomial in $\log(p)$ every time we use the term polynomial. 
